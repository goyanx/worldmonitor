...
Facts reconciled: 2026-09-26 — see Data Coverage below for how each figure is derived.
...
Coverage reconciled: 2026-09-26. Every figure below is generated from this repository's authoritative registries by `npm run build:ai-search` — the same registries that produce https://www.worldmonitor.app/sources/.

- 759 active data providers across 773 observed source hosts (342 structured/API, 463 news & OSINT feed, 30 operational-status; a host can be more than one), grouped into 10 signal domains — full catalog at https://www.worldmonitor.app/sources/
- 731 feed definitions in the shared feed registry — distinct from the 463 feed-publishing hosts above, since one host can back several feed definitions
